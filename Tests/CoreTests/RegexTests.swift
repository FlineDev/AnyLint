...
class RegexTests: XCTestCase {
  ...
  // MARK: - Options
  func testOptions() {
    let regexOptions1: Regex.Options = [
      .ignoreCase, .ignoreMetacharacters, .anchorsMatchLines, .dotMatchesLineSeparators,
    ]
    let nsRegexOptions1: NSRegularExpression.Options = [
      .caseInsensitive, .ignoreMetacharacters, .anchorsMatchLines, .dotMatchesLineSeparators,
    ]
  ...
  func testReplacingMatchesWithSpecialCharacters() {
    ...
    let stringAfterReplace1 = regex?
      .replacingMatches(in: testString, with: "$1\(NSRegularExpression.escapedTemplate(for: newValue))$3")
  ...
  func testRegexCustomStringConvertible() {
    ...
    XCTAssertEqual(regex?.description, #"/foo/"#)
  }
