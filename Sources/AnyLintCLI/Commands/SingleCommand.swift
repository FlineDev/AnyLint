...
class SingleCommand: Command {
  ...
  @Flag(
    "-x",
    "--xcode",
    description: "Prints warnings & errors in a format to be reported right within Xcodes left sidebar"
  )
  var xcode: Bool
  ...
  @Flag(
    "-d",
    "--debug",
    description: "Logs much more detailed information about what AnyLint is doing for debugging purposes"
  )
  var debug: Bool
  ...
  @Flag(
    "-l",
    "--validate",
    description: "Runs only validations for `matchingExamples`, `nonMatchingExamples` and `autoCorrectExamples`."
  )
  var validate: Bool
  ...
  @Key(
    "-i",
    "--init",
    description: "Configure AnyLint with a default template. Has to be one of: [\(CLIConstants.initTemplateCases)]"
  )
  ...
  func execute() throws {
    ...
    if let initTemplateName = initTemplateName {
      guard let initTemplate = InitTask.Template(rawValue: initTemplateName) else {
        log.message(
          "Unknown default template '\(initTemplateName)' – use one of: [\(CLIConstants.initTemplateCases)]",
          level: .error
        )
    ...
    for configPath in configurationPaths {
      do {
        try LintTask(configFilePath: configPath, logDebugLevel: debug, failOnWarnings: strict, validateOnly: validate)
          .perform()
      }
